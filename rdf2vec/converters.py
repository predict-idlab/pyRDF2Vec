...
def rdflib_to_kg(file, filetype='nt', label_predicates=[]):
    import rdflib
...
def endpoint_to_kg(endpoint_url="http://localhost:5820/db/query?query=", label_predicates=[]):
    import urllib
    import requests
    ...
    triples = [(row['s']['value'], row['p']['value'], row['o']['value']) for row in qres]
