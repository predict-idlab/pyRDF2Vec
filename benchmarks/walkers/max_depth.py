...
from pyrdf2vec.walkers import (AnonymousWalker, CommunityWalker, HALKWalker,
                               NGramWalker, RandomWalker, WalkletWalker)

dcemd_to_avg_stdev = {}
RANDOM_STATE = 22

for db, walker_name, max_depth, max_walks in itertools.product(
    ["dbpedia", "am", "mutag"],
    [
        # "AnonymousWalker",
        "CommunityWalker",
    ...
    walker = AnonymousWalker(max_depth, max_walks)
    if walker_name == "CommunityWalker":
        walker = CommunityWalker(
            max_depth, max_walks, UniformSampler(), n_jobs=4
        )
    elif walker_name == "HALKWalker":
        walker = HALKWalker(max_depth, max_walks, UniformSampler(), n_jobs=4)
    elif walker_name == "NGramWalker":
        walker = NGramWalker(max_depth, max_walks, UniformSampler(), n_jobs=4)
    elif walker_name == "RandomWalker":
        walker = RandomWalker(max_depth, max_walks, UniformSampler(), n_jobs=4)
    elif walker_name == "WalkletWalker":
        walker = WalkletWalker(
            max_depth, max_walks, UniformSampler(), n_jobs=4
        )
    elif walker_name == "SplitWalker":
        walker = SplitWalker(max_depth, max_walks, UniformSampler(), n_jobs=4)
    ...
    for _ in tqdm(range(3)):
        embeddings, _ = RDF2VecTransformer(
            Word2Vec(workers=1), walkers=[walker]
        ).fit_transform(
            KG(
        ...
        dcemd_to_avg_stdev[(db, walker_name, max_depth, max_walks)] = avg_stdev
